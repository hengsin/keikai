...
//	}
	...
	static{
		if("true".equalsIgnoreCase(Library.getProperty("io.keikai.model.internal.CollumnArrayCheck"))){
		...
		for (CellRegion mrng:getMergedRegions()) {
	        ...
	        if (t == top && l <= right && l >= left) { // to be add filter column to hide button
	        	for(int c = l; c < r; ++c) {
		        	...
		        	final SAutoFilter.NFilterColumn col = _autoFilter.getFilterColumn(colId, true);
